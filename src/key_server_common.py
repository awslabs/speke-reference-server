	...
	def fill_request(self):
		...
		for drm_system in self.root.findall("./{urn:dashif:org:cpix}DRMSystemList/{urn:dashif:org:cpix}DRMSystem"):
			...
			if system_id == HLS_AES_128_SYSTEM_ID:
				ext_x_key = get_client_url(self.client_url_prefix, content_id, kid)
				...
				drm_system.find("{urn:dashif:org:cpix}PSSH").text = CENC_PSSH_BOX
				drm_system.remove(drm_system.find("{urn:dashif:org:cpix}ContentProtectionData"))
				drm_system.remove(drm_system.find("{urn:aws:amazon:com:speke}KeyFormat"))
				drm_system.remove(drm_system.find("{urn:aws:amazon:com:speke}KeyFormatVersions"))
				drm_system.remove(drm_system.find("{urn:aws:amazon:com:speke}ProtectionHeader"))
				drm_system.remove(drm_system.find("{urn:dashif:org:cpix}URIExtXKey"))
		...
		for content_key in self.root.findall("./{urn:dashif:org:cpix}ContentKeyList/{urn:dashif:org:cpix}ContentKey"):
			...
			# base64 encoded key
			encoded_key = base64.b64encode(get_digest(KEY_STRING, content_id, kid)).decode('utf-8')
			# set on XML element
			...
			if iv is None and system_ids.get(HLS_SAMPLE_AES_SYSTEM_ID, False) == kid:
				content_key.set('explicitIV', base64.b64encode(get_digest('too much tuna!', content_id, kid)).decode('utf-8'))

	def get_response(self):
		self.fill_request()
		if self.error_message:
			return {"isBase64Encoded": False, "statusCode": 500, "headers": {"Content-Type": "text/plain"}, "body": self.error_message}
		return {"isBase64Encoded": False, "statusCode": 200, "headers": {"Content-Type": "application/xml", "Speke-User-Agent": "AWSElementalMockKeyServer"}, "body": ET.tostring(self.root).decode('utf-8')}
